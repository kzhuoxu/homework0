...
######## CheckPoint1 ##############
student_filepath="datasets/housing/housing_paml.csv"
...
def test_impute_zero():
    ...
    e_X = e_dataframe.loc[:, ~e_dataframe.columns.isin(['median_house_value'])]
    s_zero_df = B_Preprocess_Data.impute_dataset(e_X, 'Zero')
...
def test_impute_median():
    ...
    s_median_df = B_Preprocess_Data.impute_dataset(e_X, 'Median')
...
def test_impute_mean():
    ...
    e_X = e_dataframe.loc[:, ~e_dataframe.columns.isin(['median_house_value'])]
    s_mean_df = B_Preprocess_Data.impute_dataset(e_X, 'Mean')
